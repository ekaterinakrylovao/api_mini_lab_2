...
const switchToSignIn = document.getElementById('sign_in_link');
...
// Обработчик события для поля first_name
const firstName = document.getElementById(first_name_id);
firstName.addEventListener('input', () => {
  setFormValue(first_name_id, firstName.value);
  updateSignUpBtnStatus();
});

// Обработчик события для поля last_name
const lastName = document.getElementById(last_name_id);
lastName.addEventListener('input', () => {
  setFormValue(last_name_id, lastName.value);
  updateSignUpBtnStatus();
});
...
emailField.addEventListener('input', () => {
  setFormValue(email_id, emailField.value, validateEmail);
  updateSignUpBtnStatus();
});

// Обработчик события для поля password
const passwordField1 = document.getElementById(password_id);
passwordField1.addEventListener('input', () => {
  setFormValue(password_id, passwordField1.value, validatePassword);
  updateSignUpBtnStatus();
});

// Функция для обновления статуса кнопки
function updateSignUpBtnStatus() {
  signUpBtn.disabled = !getValidationStatus();
}

export { updateSignUpBtnStatus };
